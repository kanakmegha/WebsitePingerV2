	...
	<div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
		...
		<div class="flex items-center gap-2 sm:gap-3">
			{#if $isAuthenticated}
				...
				<a
					href="/add-monitor"
					class="hidden sm:inline-flex items-center gap-2 rounded-lg bg-emerald-500 px-3.5 py-2 text-xs font-semibold text-slate-950 shadow-md shadow-emerald-500/20 transition hover:bg-emerald-400 active:scale-95"
				>
		...
		<nav class="absolute left-0 right-0 top-full z-50 border-b border-slate-800 bg-slate-950 p-4 shadow-2xl md:hidden space-y-4">
			...
			<div class="pt-2 border-t border-slate-800 space-y-3">
				<a
					href="/add-monitor"
					onclick={() => (isMobileMenuOpen = false)}
					class="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 py-3 text-xs font-bold text-slate-950 shadow-md shadow-emerald-500/20 active:scale-95 transition"
				>
					<Plus class="h-4 w-4 stroke-[3]" />
					New Monitor
				</a>
