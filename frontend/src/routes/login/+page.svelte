<script lang="ts">
	...
	import { Activity, LogIn, AlertCircle, Lock, Mail } from '@lucide/svelte';
...
<div class="mx-auto max-w-md space-y-8 py-12">
	<div class="text-center space-y-2">
		...
		<h1 class="text-2xl font-extrabold text-white">Sign In to Pinger</h1>
		<p class="text-xs text-slate-400">Manage multi-tenant monitors and real-time alerts</p>
	</div>
	...
	<form onsubmit={handleSubmit} class="rounded-xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur shadow-xl space-y-5">
		...
		<div>
			<label for="login-email" class="block text-xs font-bold uppercase tracking-wider text-slate-300">
				Email Address
			</label>
		...
		<div>
			<label for="login-pass" class="block text-xs font-bold uppercase tracking-wider text-slate-300">
				Password
			</label>
			<div class="relative mt-2">
		...
		<div class="text-center pt-2">
			<p class="text-xs text-slate-400">
				...
				<a href="/register" class="font-bold text-emerald-400 hover:underline ml-1">Create Account</a>
			</p>
		</div>
	</form>
</div>
