...
func (aq *AlertQuery) QueryMonitor() *MonitorQuery {
...
func (aq *AlertQuery) QueryEvents() *AlertEventQuery {
	query := (&AlertEventClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AlertQuery) Clone() *AlertQuery {
	...
	return &AlertQuery{
		...
		withMonitor: aq.withMonitor.Clone(),
...
func (aq *AlertQuery) WithMonitor(opts ...func(*MonitorQuery)) *AlertQuery {
	query := (&MonitorClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withMonitor = query
	return aq
}

// WithEvents tells the query-builder to eager-load the nodes that are connected to
// the "events" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AlertQuery) WithEvents(opts ...func(*AlertEventQuery)) *AlertQuery {
	query := (&AlertEventClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AlertQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Alert, error) {
	...
	if query := aq.withMonitor; query != nil {
		if err := aq.loadMonitor(ctx, query, nodes, nil,
			func(n *Alert, e *Monitor) { n.Edges.Monitor = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AlertQuery) loadMonitor(ctx context.Context, query *MonitorQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *Monitor)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Alert)
	for i := range nodes {
...
func (aq *AlertQuery) loadEvents(ctx context.Context, query *AlertEventQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *AlertEvent)) error {
...
func (aq *AlertQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withMonitor != nil {
			_spec.Node.AddColumnOnce(alert.FieldMonitorID)
		}
	}
