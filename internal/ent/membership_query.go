...
func (mq *MembershipQuery) QueryTenant() *TenantQuery {
...
func (mq *MembershipQuery) WithTenant(opts ...func(*TenantQuery)) *MembershipQuery {
	query := (&TenantClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Membership, error) {
	...
	if query := mq.withTenant; query != nil {
		if err := mq.loadTenant(ctx, query, nodes, nil,
			func(n *Membership, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MembershipQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *Tenant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Membership)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (mq *MembershipQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withTenant != nil {
			_spec.Node.AddColumnOnce(membership.FieldTenantID)
		}
	}
