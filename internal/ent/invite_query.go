...
func (iq *InviteQuery) QueryTenant() *TenantQuery {
...
func (iq *InviteQuery) WithTenant(opts ...func(*TenantQuery)) *InviteQuery {
	query := (&TenantClient{config: iq.config}).Query()
...
func (iq *InviteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invite, error) {
	...
	if query := iq.withTenant; query != nil {
		if err := iq.loadTenant(ctx, query, nodes, nil,
			func(n *Invite, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InviteQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Invite, init func(*Invite), assign func(*Invite, *Tenant)) error {
...
func (iq *InviteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := iq.ctx.Fields; len(fields) > 0 {
		...
		if iq.withTenant != nil {
			_spec.Node.AddColumnOnce(invite.FieldTenantID)
		}
	}
