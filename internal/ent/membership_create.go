...
func (mc *MembershipCreate) SetTenantID(u uuid.UUID) *MembershipCreate {
...
func (mc *MembershipCreate) SetTenant(t *Tenant) *MembershipCreate {
...
func (mc *MembershipCreate) check() error {
	...
	if _, ok := mc.mutation.TenantID(); !ok {
		return &ValidationError{Name: "tenant_id", err: errors.New(`ent: missing required field "Membership.tenant_id"`)}
	}
	if _, ok := mc.mutation.Role(); !ok {
	...
	if len(mc.mutation.TenantIDs()) == 0 {
		return &ValidationError{Name: "tenant", err: errors.New(`ent: missing required edge "Membership.tenant"`)}
	}
	return nil
}
